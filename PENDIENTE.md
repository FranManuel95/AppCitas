...
| 2 | 🔴 **Pegar el catch-up de migraciones (2→46)** | Las olas W, X, Y y Z añadieron las migraciones 27…46 (facturas, sellos, membresías, calendario, sedes, galería/dominio, índices, watch channels, nota interna, buffers, consentimiento/win-back, comisiones, sede en lista de espera, idioma del usuario, observabilidad del cron + índices de purga, avisos al equipo, cierres por rango y precio/duración por empleado). `supabase-catchup.sql` es idempotente: una pasada lo deja todo al día. | `scripts/supabase-catchup.sql` |
